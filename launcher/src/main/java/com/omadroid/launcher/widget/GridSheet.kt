package com.omadroid.launcher.widget

import android.content.Context
import android.view.Gravity
import android.view.View
import android.view.ViewGroup
import android.widget.FrameLayout
import com.omadroid.launcher.NavRoute
import com.omadroid.launcher.NavStack

class GridSheet(
    context: Context,
    style: GridStyle,
) : Stack(context, style, Direction.Vertical) {
    var stack: NavStack = NavStack()
        private set

    var render: (NavRoute) -> View = { _ -> View(context) }
    var onChanged: (NavStack) -> Unit = {}

    private val pages = NodeHost()
    private var page: Node? = null
    private val header: Stack
    private val titleView: GridText
    private val backButton: GridIconButton
    private val body: GridPane

    val isOpen: Boolean
        get() = stack.isOpen && visibility == VISIBLE

    init {
        setBackgroundColor(style.colors.background)
        importantForAccessibility = IMPORTANT_FOR_ACCESSIBILITY_YES
        overScrollMode = OVER_SCROLL_NEVER
        visibility = GONE
        header =
            Stack(context, style, Direction.Horizontal).apply {
                contentDescription = "Sheet"
                isClickable = true
                setOnClickListener { popOrDismiss() }
                setBackgroundColor(style.colors.lighterBackground)
            }
        backButton =
            GridIconButton(context, style).apply {
                text = IconGlyphs.BACK
                contentDescription = "Back"
                isClickable = true
                setOnClickListener { popOrDismiss() }
            }
        header.add(backButton, Slot.square)
        titleView =
            GridText(context, style).apply {
                gravity = Gravity.CENTER_VERTICAL or Gravity.START
            }
        header.add(titleView, Slot.grow())
        add(header, Slot.units(1))
        body =
            GridPane(context, style).apply {
                setBackgroundColor(style.colors.background)
            }
        add(body, Slot.grow())
    }

    override fun style(next: GridStyle) {
        super.style(next)
        setBackgroundColor(next.colors.background)
        body.setBackgroundColor(next.colors.background)
        header.setBackgroundColor(next.colors.lighterBackground)
        pages.style(next)
        titleView.gravity = Gravity.CENTER_VERTICAL or Gravity.START
    }

    fun show(route: NavRoute) {
        animate().cancel()
        stack = NavStack.root(route)
        bind()
        translationY = 0f
        translationX = offscreenX()
        visibility = VISIBLE
        post {
            translationX = offscreenX()
            animate().translationX(0f).setDuration(SLIDE_MS).start()
        }
        onChanged(stack)
    }

    fun restore(next: NavStack) {
        animate().cancel()
        stack = next
        bind()
        translationX = 0f
        translationY = 0f
        visibility = VISIBLE
        onChanged(stack)
    }

    fun push(route: NavRoute) {
        if (!stack.isOpen) {
            show(route)
            return
        }
        stack = stack.push(route)
        bind()
        onChanged(stack)
    }

    fun popOrDismiss(): Boolean {
        if (!stack.isOpen) {
            return false
        }
        if (stack.canPop) {
            stack = stack.pop()
            bind()
            onChanged(stack)
            return true
        }
        dismiss()
        return true
    }

    fun dismiss() {
        if (visibility != VISIBLE) {
            stack = NavStack()
            onChanged(stack)
            return
        }
        animate()
            .translationX(offscreenX())
            .setDuration(SLIDE_MS)
            .withEndAction {
                visibility = GONE
                translationX = offscreenX()
                body.removeAllViews()
                stack = NavStack()
                onChanged(stack)
            }
            .start()
    }

    private fun bind() {
        val route = stack.current
        if (route == null) {
            body.removeAllViews()
            return
        }
        titleView.text = route.title
        backButton.visibility = VISIBLE
        page?.let { pages.remove(it) }
        page = null
        body.removeAllViews()
        val content = render(route)
        if (content is Node) {
            pages.add(content)
            page = content
        }
        body.addView(
            content,
            FrameLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT,
                Gravity.TOP,
            ),
        )
    }

    private fun offscreenX(): Float {
        val measured = width
        val span =
            if (measured > 0) {
                measured
            } else {
                resources.displayMetrics.widthPixels
            }
        return -span.toFloat()
    }

    companion object {
        private const val SLIDE_MS = 200L
    }
}
