#!/usr/bin/env bash
# Gradle-build OmadroidLauncher and copy it to launcher/prebuilt for Soong import.
set -euo pipefail
...
usage() {
  cat <<EOF
Build the HOME APK with Gradle (Omadroid Compose).

Usage:
  build-launcher.sh [--aosp <aosp-root>]

Writes launcher/prebuilt/OmadroidLauncher.apk.
...
src="$(find "${ROOT}/launcher/build/outputs/apk/debug" -name '*-debug.apk' -print -quit)"
[[ -n "$src" && -f "$src" ]] || omadroid_die "debug APK missing after assembleDebug"

out_dir="${ROOT}/launcher/prebuilt"
mkdir -p "$out_dir"
out="${out_dir}/OmadroidLauncher.apk"
...
printf 'omadroid: launcher apk -> %s\n' "$out"
