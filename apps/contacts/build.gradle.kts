...
android {
    namespace = "com.omadroid.contacts"
    compileSdk = 36

    defaultConfig {
        applicationId = "com.omadroid.contacts"
        minSdk = 26
        targetSdk = 36
        versionCode = 1
        versionName = "0.1.0"
...
afterEvaluate {
    tasks.register("copyPrebuiltApk") {
        dependsOn("packageDebug")
        doLast {
            val src =
                fileTree(layout.buildDirectory.dir("outputs/apk/debug")) {
                    include("*.apk")
                }.files.firstOrNull()
                    ?: error("debug APK missing after packageDebug")
            val dest = layout.projectDirectory.file("prebuilt/OmadroidContacts.apk").asFile
            dest.parentFile.mkdirs()
            src.copyTo(dest, overwrite = true)
        }
    }
    tasks.named("assembleDebug").configure {
        finalizedBy("copyPrebuiltApk")
    }
}
